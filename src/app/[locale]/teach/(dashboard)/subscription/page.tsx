...
import {
  ...
  ChangePaymentButton,
} from "@/components/teacher/subscription-actions";
...
export default async function SubscriptionPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
      ...
      {/* Plan comparison ripped per locked-in default 4: the progressive
          revenue tier above is the only pricing model — there are no
          Starter/Pro/Agency plan cards. */}
      ...
      <section className="mt-10 grid gap-6 lg:grid-cols-[2fr_1fr]">
          ...
          <ul className="divide-y divide-border">
            {invoices.map((inv) => (
              ...
              >
                ...
                <InvoiceDownloadButton invoiceId={inv.id} label={t("invoices.download")} />
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-[var(--radius-xl)] border border-border bg-card p-5">
            ...
            <ChangePaymentButton label={t("payment.change")} />
          </div>
