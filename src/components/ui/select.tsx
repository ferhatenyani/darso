...
>(({ className, children, size = "md", ...props }, ref) => (
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="ms-auto h-4 w-4 text-ink-3 transition-transform group-data-[state=open]:rotate-180" />
...
>(({ className, children, position = "popper", sideOffset = 6, ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      {...props}
    >
      <SelectPrimitive.ScrollUpButton className="flex h-7 items-center justify-center bg-popover text-ink-3">
        <ChevronUp className="h-4 w-4" />
      </SelectPrimitive.ScrollUpButton>
      <SelectPrimitive.Viewport className="scroll-thin p-1">{children}</SelectPrimitive.Viewport>
      <SelectPrimitive.ScrollDownButton className="flex h-7 items-center justify-center bg-popover text-ink-3">
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn("px-2 py-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-3", className)}
    {...props}
  />
));
